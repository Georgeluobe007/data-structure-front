...
function HumanChangePassword() {
    const schma = yup.object().shape({
      ...
      const officeId = localStorage.getItem("humanresocesId")
      const {handleSubmit,register,formState:{errors}} = useForm({
        resolver:yupResolver(schma)
      })
      const mutationFn = (data) => {
        const response = axios.put(`http://localhost:3004/changePasswordForHr/${officeId}`,data);
         return response;
      }
...
export default HumanChangePassword